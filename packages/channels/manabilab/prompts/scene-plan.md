...
2. **hero scene（index=0）はフックの長さに比例**:
   - フックがカギ括弧（「」『』）で囲まれた引用フレーズ**単独**なら **1.5〜2.0秒**（短くテンポよく）
   - 通常の文フックなら **3.5〜4.0秒** で大きく見せる
3. **通常シーン: 2.5〜3.5秒**。単調を避けるため 2.5/3.0/3.5 を混ぜる
4. **合計秒数は `estimatedDurationSec` と一致**させる

### シーンあたりの割当
- `narration`: **1文（または2文）ごと**に1シーン。細かく刻みすぎない
- **カギ括弧（「」『』）で囲まれた引用フックフレーズは独立した1シーンに切る**。続く解説文と同じシーンにまとめない（先頭スライドが長くなるとスワイプ離脱を招くため、引用は単独で短く打って次のカットへ移る）
  - OK: scene[0] = `「一粒で何日も戦える」。` / scene[1] = `戦国時代、過酷な戦場で武士たちが携帯した兵糧丸。`
  - NG: scene[0] = `「一粒で何日も戦える」。戦国時代、過酷な戦場で武士たちが携帯した兵糧丸。`（引用と解説が同居して6秒級の長尺になる）
