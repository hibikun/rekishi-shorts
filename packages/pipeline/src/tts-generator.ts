...
const NARRATOR_STYLE_BY_CHANNEL: Record<string, string> = {
  ...
  // manabilab-canva: 学習科学 × deadpan / chill な「だるそう」感 × 早口・短い間。
  // 「やる気なさそうにクールに早口で語る」を狙う。
  // ただし「ため息・息漏れ・吐息混じり」は禁止 — chill さは "声の力感の弱さ" ではなく
  // "感情の盛り上がりの欠如" で表現させる。声自体は steady chest voice で太く保つ。
  "manabilab-canva":
    "Say the following in natural Japanese with a LOW-PITCHED, deadpan, chill, unbothered narrator's voice — like a nonchalant, slightly bored older brother casually rattling off science facts to a friend. Keep the pitch noticeably LOW and the emotional energy DETACHED and nonchalant — sound like you're not really invested in the topic. SPEAK QUICKLY and KEEP PAUSES BETWEEN PHRASES AND SENTENCES VERY SHORT — almost run sentences together, no drawn-out gaps. Keep INFLECTION MINIMAL AND FLAT: barely any emphasis on key words, no punching, no rising intonation at phrase endings. Sound knowing, unbothered, deadpan-cool — as if everything you're saying is obvious to you. CRITICALLY: deliver with STEADY, FULL CHEST VOICE — the voice itself must stay solid, supported, and audible. The 'chill / detached' feel comes from FLAT EMOTIONAL TONE, NOT from a weak or breathy voice. AVOID: enthusiasm, anchorman crispness, emphatic stress, sing-song cadence, slow plodding pace, drawn-out pauses, breath gaps between sentences. ALSO STRICTLY AVOID sighing, audible exhales, breathy or whispery delivery, vocal fry, fading-out phrase endings, heavy breaths, mumbling, muttering, or any 'sigh-like' weariness in the voice itself — the voice must sound chill but never tired-sounding:",
};
